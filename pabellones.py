...
def crear_tabla_pabellones(archivo):
    try:
        ...
        df_operativos = archivo[archivo['PABELLONES'].isin(tipos_operatividad)]

        # Generar la tabla resumen
        conteo_establecimientos = (
            df_operativos.groupby(['TIPO_ESTABLECIMIENTO', 'PABELLONES'])
            .size()
        ...
        conteo_establecimientos.rename(columns={'OPERATIVO': 'OPERATIVO_PABELLONES'}, inplace=True)
        conteo_establecimientos.rename(columns={'SEMI_OPERATIVO': 'SEMI_OPERATIVO_PABELLONES'}, inplace=True)
        conteo_establecimientos.rename(columns={'NO_OPERATIVO': 'NO_OPERATIVO_PABELLONES'}, inplace=True)
        ...
        tablas_Operatividad['principalPABELLONES'] = conteo_establecimientos_html
        ...
        for operatividad in tipos_operatividad:
            for tipo_establecimiento in archivo['TIPO_ESTABLECIMIENTO'].unique():
                ...
                df_filtrado = archivo[(archivo['TIPO_ESTABLECIMIENTO'] == tipo_establecimiento) & 
                                (archivo['PABELLONES'] == operatividad)]
                
                # Si el DataFrame no está vacío, proceder
                if not df_filtrado.empty:
                    columnas = ['NOMBRE_ESTABLECIMIENTO', 'PABELLONES']
                    ...
                    if operatividad in ['SEMI_OPERATIVO', 'NO_OPERATIVO']:
                        df_filtrado['DETALLE_PABELLONES'] = df_filtrado.get('DETALLE_PABELLONES', 'SIN_DESCRIPCION')
                        columnas.append('DETALLE_PABELLONES')

                    # Convertir la tabla filtrada en HTML
                    table_name = f"{tipo_establecimiento}_{operatividad}_PABELLONES".upper()
