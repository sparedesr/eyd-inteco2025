...
def crear_tabla_vehiculos(archivo):
    try:
        tipos_operatividad = ['OPERATIVO', 'SEMI_OPERATIVO', 'NO_OPERATIVO']
        df_operativos = archivo[archivo['VEHICULOS'].isin(tipos_operatividad)]

        # Generar la tabla resumen
        conteo_establecimientos = (
            df_operativos.groupby(['TIPO_ESTABLECIMIENTO', 'VEHICULOS'])
            .size()
        ...
        conteo_establecimientos.rename(columns={'OPERATIVO': 'OPERATIVO_VEHICULOS'}, inplace=True)
        conteo_establecimientos.rename(columns={'SEMI_OPERATIVO': 'SEMI_OPERATIVO_VEHICULOS'}, inplace=True)
        conteo_establecimientos.rename(columns={'NO_OPERATIVO': 'NO_OPERATIVO_VEHICULOS'}, inplace=True)
        ...
        tablas_Operatividad['principalVEHICULOS'] = conteo_establecimientos_html
        ...
        for operatividad in tipos_operatividad:
            for tipo_establecimiento in archivo['TIPO_ESTABLECIMIENTO'].unique():
                ...
                df_filtrado = archivo[(archivo['TIPO_ESTABLECIMIENTO'] == tipo_establecimiento) & 
                                (archivo['VEHICULOS'] == operatividad)]
                
                # Si el DataFrame no está vacío, proceder
                if not df_filtrado.empty:
                    columnas = ['NOMBRE_ESTABLECIMIENTO', 'VEHICULOS']
                    ...
                    if operatividad in ['SEMI_OPERATIVO', 'NO_OPERATIVO']:
                        df_filtrado['DETALLE_VEHICULOS'] = df_filtrado.get('DETALLE_VEHICULOS', 'SIN_DESCRIPCION')
                        columnas.append('DETALLE_VEHICULOS')

                    # Convertir la tabla filtrada en HTML
                    table_name = f"{tipo_establecimiento}_{operatividad}_VEHICULOS".upper()
