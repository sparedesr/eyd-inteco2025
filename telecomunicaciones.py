...
def crear_tabla_teleco(archivo):
    try:
        tipos_operatividad = ['SERVICIO_NORMAL', 'SERVICIO_INTERMITENTE', 'SIN_SERVICIO']
        df_operativos = archivo[archivo['TELECOMUNICACIONES'].isin(tipos_operatividad)]

        # Generar la tabla resumen
        conteo_establecimientos = (
            df_operativos.groupby(['TIPO_ESTABLECIMIENTO', 'TELECOMUNICACIONES',])
            .size()
        ...
        conteo_establecimientos.rename(columns={'SERVICIO_NORMAL': 'SERVICIO_NORMAL_TELECOMUNICACIONES'}, inplace=True)
        conteo_establecimientos.rename(columns={'SERVICIO_INTERMITENTE': 'SERVICIO_INTERMITENTE_TELECOMUNICACIONES'}, inplace=True)
        conteo_establecimientos.rename(columns={'SIN_SERVICIO': 'SIN_SERVICIO_TELECOMUNICACIONES'}, inplace=True)
        ...
        tablas_Operatividad['principalTELECOMUNICACIONES'] = conteo_establecimientos_html
        ...
        for operatividad in tipos_operatividad:
            for tipo_establecimiento in archivo['TIPO_ESTABLECIMIENTO'].unique():
                ...
                df_filtrado = archivo[(archivo['TIPO_ESTABLECIMIENTO'] == tipo_establecimiento) & 
                                (archivo['TELECOMUNICACIONES'] == operatividad)]
                
                # Si el DataFrame no está vacío, proceder
                if not df_filtrado.empty:
                    columnas = ['NOMBRE_ESTABLECIMIENTO', 'TELECOMUNICACIONES','DETALLE_TELECOMUNICACIONES']
                    ...
                    table_name = f"{tipo_establecimiento}_{operatividad}_TELECOMUNICACIONES".upper()
