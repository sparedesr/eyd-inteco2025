...
def crear_tabla_boxes(archivo):
    try:
        tipos_operatividad = ['OPERATIVO', 'SEMI_OPERATIVO', 'NO_OPERATIVO']
        df_operativos = archivo[archivo['BOXES_DE_ATENCION'].isin(tipos_operatividad)]

        # Generar la tabla resumen
        conteo_establecimientos = (
            df_operativos.groupby(['TIPO_ESTABLECIMIENTO', 'BOXES_DE_ATENCION'])
            .size()
        ...
        conteo_establecimientos.rename(columns={'OPERATIVO': 'OPERATIVO_BOXES'}, inplace=True)
        conteo_establecimientos.rename(columns={'SEMI_OPERATIVO': 'SEMI_OPERATIVO_BOXES'}, inplace=True)
        conteo_establecimientos.rename(columns={'NO_OPERATIVO': 'NO_OPERATIVO_BOXES'}, inplace=True)
        ...
        tablas_Operatividad['principalBoxes'] = conteo_establecimientos_html
        ...
        for operatividad in tipos_operatividad:
            for tipo_establecimiento in archivo['TIPO_ESTABLECIMIENTO'].unique():
                ...
                df_filtrado = archivo[(archivo['TIPO_ESTABLECIMIENTO'] == tipo_establecimiento) & 
                                (archivo['BOXES_DE_ATENCION'] == operatividad)]
                
                # Si el DataFrame no está vacío, proceder
                if not df_filtrado.empty:
                    columnas = ['NOMBRE_ESTABLECIMIENTO', 'BOXES_DE_ATENCION']
                    ...
                    if operatividad in ['SEMI_OPERATIVO', 'NO_OPERATIVO']:
                        df_filtrado['DETALLE_BOXES_DE_ATENCION'] = df_filtrado.get('DETALLE_BOXES_DE_ATENCION', 'SIN_DESCRIPCION')
                        columnas.append('DETALLE_BOXES_DE_ATENCION')

                    # Convertir la tabla filtrada en HTML
                    table_name = f"{tipo_establecimiento}_{operatividad}_BOXES".upper()
