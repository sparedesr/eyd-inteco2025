...
def crear_tabla_urgencia(archivo):
    try:
        tipos_operatividad = ['OPERATIVO', 'SEMI_OPERATIVO', 'NO_OPERATIVO']
        df_operativos = archivo[archivo['URGENCIA'].isin(tipos_operatividad)]

        # Generar la tabla resumen
        conteo_establecimientos = (
            df_operativos.groupby(['TIPO_ESTABLECIMIENTO', 'URGENCIA'])
            .size()
        ...
        conteo_establecimientos.rename(columns={'OPERATIVO': 'OPERATIVO_URGENCIA'}, inplace=True)
        conteo_establecimientos.rename(columns={'SEMI_OPERATIVO': 'SEMI_OPERATIVO_URGENCIA'}, inplace=True)
        conteo_establecimientos.rename(columns={'NO_OPERATIVO': 'NO_OPERATIVO_URGENCIA'}, inplace=True)
        ...
        tablas_Operatividad['principalURGENCIA'] = conteo_establecimientos_html
        ...
        for operatividad in tipos_operatividad:
            for tipo_establecimiento in archivo['TIPO_ESTABLECIMIENTO'].unique():
                ...
                df_filtrado = archivo[(archivo['TIPO_ESTABLECIMIENTO'] == tipo_establecimiento) & 
                                (archivo['URGENCIA'] == operatividad)]
                
                # Si el DataFrame no está vacío, proceder
                if not df_filtrado.empty:
                    columnas = ['NOMBRE_ESTABLECIMIENTO', 'URGENCIA']
                    ...
                    if operatividad in ['SEMI_OPERATIVO', 'NO_OPERATIVO']:
                        df_filtrado['DETALLE_URGENCIA'] = df_filtrado.get('DETALLE_URGENCIA', 'SIN_DESCRIPCION')
                        columnas.append('DETALLE_URGENCIA')

                    # Convertir la tabla filtrada en HTML
                    table_name = f"{tipo_establecimiento}_{operatividad}_URGENCIA".upper()
